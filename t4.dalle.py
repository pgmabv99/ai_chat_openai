...
def image_get(prompt):
    ...
    img_filename = images_dt.strftime('output/DALLE-%Y%m%d_%H%M%S')  # like 'DALLE-20231111_144356'
...
#  "Subject: ballet dancers posing on a beam. "  # use the space at end
 "Subject: show 2 athletes who won the gold medal in curling at the 2022 Winter Olympics "  # use the space at end
 "Style: romantic impressionist painting."     # this is implicit line continuation
)
image_objects=image_get(prompt)
image_disp(image_objects)


prompt = (
#  "Subject: ballet dancers posing on a beam. "  # use the space at end
 " add 10 people to previous image"
)
image_objects=image_get(prompt)
image_disp(image_objects)